...
async function boot(){
  try{
    // Discord callback arrives at GitHub Pages as ?auth=TOKEN.
    // Also accept the old #auth=TOKEN format for backwards compatibility.
    const query=new URLSearchParams(location.search);
    const hash=new URLSearchParams(location.hash.startsWith('#')?location.hash.slice(1):location.hash);
    const oauthToken=query.get('auth') || hash.get('auth');
    if(oauthToken){
      localStorage.setItem(AUTH_KEY,oauthToken);
      const cleanUrl=`${location.origin}${location.pathname}#home`;
    ...
    // Do NOT erase a stored session because of a transient network/CORS error.
    // Only authentication failures invalidate the local bearer token.
