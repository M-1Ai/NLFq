...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const failUrl = `${config.frontendUrl.replace(/\/$/, '')}/#home`;
    const successUrl = `${config.frontendUrl.replace(/\/$/, '')}/`;
    ...
    try {
      ...
      // Keep the session cookie as a fallback, but pass the JWT through the
      // query string so GitHub Pages can reliably receive it after OAuth.
      // The frontend immediately removes the token from the visible URL.
      res.cookie(config.cookieName, token, config.sessionCookieOptions);
      const separator = config.frontendUrl.includes('?') ? '&' : '?';
      return res.redirect(`${successUrl}${separator}auth=${encodeURIComponent(token)}#home`);
    } catch (e) {
...
router.get('/status', (req, res) => {
  res.json({ ok: true, authenticated: Boolean(req.cookies[config.cookieName]) });
});
